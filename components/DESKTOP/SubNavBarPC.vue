<template>
  <div
    class="h-max w-full py-[1.2vh] bg-[#000] bg-opacity-[.3] border-b-[2px] border-[#fff1] backdrop-blur-[8px] flex justify-between px-[2vw] font-popins"
  >
    <span
      class="text-[.6rem] tracking-[.1vw] font-[500] text-[#fff9] flex items-center gap-[.4vw] hover:text-[#f1f1f1] ease-in-out transition-all duration-[.4s] cursor-pointer"
      ><svg
        xmlns="http://www.w3.org/2000/svg"
        class="w-[.9rem]"
        viewBox="0 0 24 24"
      >
        <path
          fill="currentColor"
          d="M12.394 16.958c0-.789.338-.902 1.127-.451a54 54 0 0 0 2.704 1.465c0-.45.451-.789 1.24-.564s1.577.338 1.577.338s-.45-1.014-.676-1.464c-.338-.789 0-1.24.338-1.352c0 0-.45-.338-.789-.564c-.676-.45-.563-1.014.113-1.24c.902-.45 2.141-.9 2.141-.9c-.338-.226-.789-.79-.338-1.578c.45-.676 1.24-1.69 1.24-1.69H18.93c-.79 0-1.015-.676-1.015-1.127c0 0-1.239.901-2.14 1.465c-.79.563-1.465 0-1.352-.902a37 37 0 0 0 .338-2.93c-.451.451-1.24.339-1.69-.337c-.564-1.127-1.127-2.48-1.127-2.48S11.38 4 10.817 5.128c-.338.676-1.127.788-1.578.45a37 37 0 0 0 .338 2.93c.113.789-.563 1.352-1.352.789c-.901-.564-2.253-1.465-2.253-1.465c0 .45-.226 1.014-1.014 1.127H2.817s.789 1.014 1.24 1.69s0 1.352-.339 1.577c0 0 1.127.564 2.141.902c.676.338.902.788.113 1.24c-.226.225-.789.563-.789.563c.45.112.789.563.45 1.352c-.225.45-.675 1.464-.675 1.464s.788-.225 1.577-.338c.789-.225 1.127.226 1.24.564c0 0 1.352-.789 2.704-1.465c.676-.45 1.127-.225 1.127.45v1.916c0 1.127-.226 2.254-.564 2.93c-5.07-.564-9.352-4.62-9.352-10.028c0-5.521 4.62-10.029 10.366-10.029c5.747 0 10.367 4.508 10.367 10.029c0 5.183-4.057 9.464-9.24 10.028v1.352C19.268 22.592 24 17.746 24 11.775C24 5.352 18.592.282 11.944.282C5.408.282 0 5.352 0 11.662c0 5.521 4.169 10.14 9.69 11.155c.902.225 1.465.338 2.028.901c.564-1.126.676-3.38.676-4.62Z"
        ></path>
      </svg>
      RENOVATION REIMAGINED</span
    >
    <div
      class="socialsDiv h-max w-max flex gap-[2vw] text-[.6rem] tracking-[.1vw] font-[400] text-[#fff9]"
    >
      <a
        href="#"
        class="flex items-center gap-[.4vw] hover:text-[#f1f1f1] ease-in-out transition-all duration-[.4s]"
        >INSTAGRAM<svg
          xmlns="http://www.w3.org/2000/svg"
          class="w-[1rem]"
          viewBox="0 0 24 24"
        >
          <path
            fill="currentColor"
            fill-rule="evenodd"
            d="M11.376 2.016h6.608v6.608h-1.5V4.577l-5.87 5.87l-.53.53l-1.061-1.06l.53-.53l5.87-5.87h-4.047zM5 5.5h4V4H5a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2-2v-4h-1.5v4a.5.5 0 0 1-.5.5H5a.5.5 0 0 1-.5-.5V6a.5.5 0 0 1 .5-.5"
            clip-rule="evenodd"
          ></path></svg
      ></a>
      <a
        href="#"
        class="flex items-center gap-[.4vw] hover:text-[#f1f1f1] ease-in-out transition-all duration-[.4s]"
        >FACEBOOK<svg
          xmlns="http://www.w3.org/2000/svg"
          class="w-[1rem]"
          viewBox="0 0 24 24"
        >
          <path
            fill="currentColor"
            fill-rule="evenodd"
            d="M11.376 2.016h6.608v6.608h-1.5V4.577l-5.87 5.87l-.53.53l-1.061-1.06l.53-.53l5.87-5.87h-4.047zM5 5.5h4V4H5a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2-2v-4h-1.5v4a.5.5 0 0 1-.5.5H5a.5.5 0 0 1-.5-.5V6a.5.5 0 0 1 .5-.5"
            clip-rule="evenodd"
          ></path></svg
      ></a>
      <a
        href="#"
        class="flex items-center gap-[.4vw] hover:text-[#f1f1f1] ease-in-out transition-all duration-[.4s]"
        >LINKEDIN<svg
          xmlns="http://www.w3.org/2000/svg"
          class="w-[1rem]"
          viewBox="0 0 24 24"
        >
          <path
            fill="currentColor"
            fill-rule="evenodd"
            d="M11.376 2.016h6.608v6.608h-1.5V4.577l-5.87 5.87l-.53.53l-1.061-1.06l.53-.53l5.87-5.87h-4.047zM5 5.5h4V4H5a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2-2v-4h-1.5v4a.5.5 0 0 1-.5.5H5a.5.5 0 0 1-.5-.5V6a.5.5 0 0 1 .5-.5"
            clip-rule="evenodd"
          ></path></svg
      ></a>
    </div>
  </div>
</template>

<script setup></script>

<style lang="scss" scoped></style>
